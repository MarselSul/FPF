...
- **Suite:** [Purpose, membership and edition rules](FOUNDATIONAL-THINKING-DPF-SUITE.md)
...
| **Computational Thinking** | Construct a way to obtain an answer or behavior from represented inputs, accounting for progress and resources. | An obtaining procedure, its representation and conditions, or an obstruction requiring a different demand. |
...
FPF B.5.RC and B.5.RA recover a construction or argument. MATH.1/.5 builds and interprets operation sequences; [MATH.2](MATHEMATICAL-PRACTICE-DPF.md#math2---form-a-quotient-that-preserves-operations) tests whether identifying descriptions preserves the required operations and answer. MATH.16 already treats functions as objects with evaluation maps. The MATH Readme's MP-COMPOSE, MP-IDENTIFY and MP-EXTEND entries expose these available uses.
...
The planned MATH.17/.18 and ME.6.MC/ME.25 will develop operation transformations, changes of interpretation and further methodological construction. The present sequence can already expose a failed replacement and say what it must preserve. It does not yet supply every such transformation.
...
General inferential construction, model discrimination, causal identification and compressed sequential-decision state are selected MMP additions. Use an existing suitable subject method or source for such a remaining operation; the current five-body DPF does not supply it merely by mentioning it.
...
Broader approximation and bounding methods, numerical construction, surrogate construction and physical-resource comparisons are planned in MATH, MMP, CMP and PHY. Use their current FPF suppliers or direct specialist methods where applicable; an unavailable algorithm or physical premise remains an explicit limit on this answer.
...
For acquiring or providing the needed capability, use the [Engineering DPF Suite](Engineering%20DPF%20Suite/README.md) entries for Human Capability Development, development opportunities and Explanation Design. Their methods address learning, support and transfer. A description of a construction alone does not establish that a particular learner can use it. Planned MATH.23 and the further notational methods will add disciplinary instructions for continued problem development and recoverable expression.
...
The selection contains **65 planned or existing pattern bodies in five DPFs**. Of these, **18 are published here**: thirteen in MATH and five in MMP. No PHY, CMP or NOT edition is offered in this Reference. The numbers describe this selected repertoire, not a completeness claim about the fields.

| DPF | Available contribution | Selected expansion |
| --- | --- | --- |
| Mathematical Thinking: 20 bodies selected | 13 bodies: choosing constructions through maps; paths, quotients, interpretation and transport; witnesses, proof extraction and countermodels; invariants, symmetry and variation. | 7 bodies: spaces of operations and their transformations; comparison through interpretations; proofs through intermediate claims; bounds; convergent approximations; changes of axioms and their consequences; and conjectures developed by changing a construction. |
...
Each expansion must supply reusable operations and their connections. Completing the list would still leave source coverage open to a newly found difficulty or method. A result that no current body can produce calls for a specific added or changed method and placement.
...
**The Reference preserves connections that a catalogue loses.** A supplied mathematical object does not determine which observation refers to it. A physical law does not by itself supply an affordable calculation. An obtaining procedure must return something the receiving question can use. The entries therefore state intermediate results, conditions, failures and returns. FPF B.5.MPC and B.5.MPC.R carry the common coordination and repair methods. This Reference explains selected uses of them.
